...
$website_san = cmtx_sanitize($website);
...
if ($cmtx_settings->show_flag) {
	$unflagged = false;
	$query = mysql_query("SELECT * FROM `" . $cmtx_mysql_table_prefix . "reports` WHERE `status` = 'pending' AND `comment_id` = '$id_san'");
	while ($reports = mysql_fetch_assoc($query)) {
		if (isset($_POST['report_' . $reports["id"]])) {
			$report_id = $reports["id"];
			if ($_POST['report_' . $reports["id"]] == "good") {
				mysql_query("UPDATE `" . $cmtx_mysql_table_prefix . "reports` SET `status` = 'good' WHERE `id` = '$report_id'");
			} else if ($_POST['report_' . $reports["id"]] == "fair") {
				mysql_query("UPDATE `" . $cmtx_mysql_table_prefix . "reports` SET `status` = 'fair' WHERE `id` = '$report_id'");
			} else {
				mysql_query("UPDATE `" . $cmtx_mysql_table_prefix . "reports` SET `status` = 'bad' WHERE `id` = '$report_id'");
				$ip_address_query = mysql_query("SELECT `ip_address` FROM `" . $cmtx_mysql_table_prefix . "reports` WHERE `id` = '$report_id'");
				$ip_address_result = mysql_fetch_assoc($ip_address_query);
				$ip_address = $ip_address_result["ip_address"];
				mysql_query("INSERT INTO `" . $cmtx_mysql_table_prefix . "bans` (`ip_address`, `reason`, `unban`, `dated`) VALUES ('$ip_address', '" . cmtx_sanitize(CMTX_FIELD_VALUE_BAD_REPORT) . "', '0', NOW());");
			}
		$unflagged = true;
		}
	}
	if ($unflagged) {
		mysql_query("UPDATE `" . $cmtx_mysql_table_prefix . "comments` SET `is_flagged` = '0' WHERE `id` = '$id_san'");
	}
}
...
if ($country != "blank") {
	$countries = str_ireplace('"'.$country.'"', '"'.$country.'" selected="selected"', $countries);
}
echo $countries;
?>
<p />
...
if ($rating != "blank") {
	$ratings = str_ireplace('"'.$rating.'"', '"'.$rating.'" selected="selected"', $ratings);
}
echo $ratings;
?>
<p />
