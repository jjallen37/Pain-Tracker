...
<?php
        ...
        if( mysql_num_rows($sql) == 0 )
	    {
				...
				?>
				<div>
				<p> You do not yet have a survey!  Please <a href="track.php?loc=default_survey">click here</a>.
	    ...
	    else
	    {
            ...
            {
                $newTz = $_POST['ddtz'];
                $query = "update User set time_zone = $newTz where userid = $user_id";
	        ...
	        $tz = $_SESSION['tz'];
            ?>
            <p>Here you can take or modify your personalized tracking survey.</p>
			...
			if($row["category_name"] != "Note")
			{
			if($divisby2 % 2 == 0 AND $divisby2 != 0)
			{
			$startpostop = $startpostop + 300;
			$startposleft = 340;
			}
			?>
			
			<div id="anything" style="border-style: solid; border-color: red; position: absolute; z-index: 1; top: <?php echo $startpostop ?>px; left: <?php echo $startposleft ?>px; width: 250px;>
			...
			<?php
				echo " 0-";
				echo $row["category_scale1"];
			?>
			</li>
			<li class="ui-widget-content">
			<?php
				echo " 1-";
				echo $row["category_scale2"];
			?>
			</li>
			<li class="ui-widget-content">
			<?php
			echo " 2-";
			echo $row["category_scale3"];
			?>
			</li>
			<li class="ui-widget-content">
			<?php
			echo " 3-";
			echo $row["category_scale4"];
			?>
			</li>
			<li class="ui-widget-content">
			<?php
			echo " 4-";
			echo $row["category_scale5"];
			?>
			</li>
			<li class="ui-widget-content">
			<?php
			echo " 5-";
			echo $row["category_scale6"];
			?>
			</li>
			<li class="ui-widget-content">
			<?php
			echo " 6-";
			echo $row["category_scale7"];
			?>
			</li>
			<li class="ui-widget-content">
			<?php
			echo " 7-";
			echo $row["category_scale8"];
			?>
			</li>
			<li class="ui-widget-content">
			<?php
			echo " 8-";
			echo $row["category_scale9"];
			?>
			</li>
			<li class="ui-widget-content">
			<?php
			echo " 9-";
			echo $row["category_scale10"];
			?>
			</li>
			<li class="ui-widget-content">
			<?php
			echo "10-";
			echo $row["category_scale11"];
			?>
			</li>
			</ul>
			</div>
			<?php
			$startposleft = $startposleft + 300;
			$divisby2 = $divisby2+1;
			}
			}
			?>
			...
			<p><input type="submit" name="action" value="Submit Survey!" /></p>
			<br><br><br><br><br>
	...
	/*else if( $_POST['action'] == "Add Category" )
	{
		...
		else if($_POST['step'] == "1" ) 
		{
			...
			else
			{
				...
				<?php
				$query = "select fields from User_Survey where category = '$_POST[cat]' group by fields order by fields asc";
				$sql = mysql_query($query) or handle_error(mysql_error());
