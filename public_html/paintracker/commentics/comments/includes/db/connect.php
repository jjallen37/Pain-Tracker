...
if (!$cmtx_connection) {
	...
	echo "Sorry, there is a database connection problem.<p />Please check back again shortly.";
	$cmtx_db_ok = false;
	return;
}
...
if (!$cmtx_database) {
	echo "<h3>Commentics</h3>";
	echo "<div style='margin-bottom: 10px;'></div>";
	echo "Sorry, there is a database selection problem.<p />Please check back again shortly.";
	$cmtx_db_ok = false;
	return;
}
...
if (mysql_num_rows(mysql_query("SHOW TABLES LIKE '" . $cmtx_mysql_table_prefix . "comments'")) == 0) {
	if (!defined("CMTX_IN_INSTALLER")) {
		echo "<h3>Commentics</h3>";
		echo "<div style='margin-bottom: 10px;'></div>";
		echo "Sorry, there is a database tables problem.<p />Please check back again shortly.";
		$cmtx_db_ok = false;
		return;
	}
}
